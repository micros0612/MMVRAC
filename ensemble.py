...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset',
                        required=True,
                        choices={'uav/xsub_v1','uav/xsub_v2','ntu/xsub', 'ntu/xview', 'ntu120/xsub', 'ntu120/xset', 'NW-UCLA'},
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub_v1' in arg.dataset:
            npz_data = np.load('./data/' + 'uav/' + 'UAV_XSub_V1.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xsub_v2' in arg.dataset:
            npz_data = np.load('./data/' + 'uav/' + 'UAV_XSub_V2.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
