...
fail_logger_length = logging.getLogger('noise_outliers_length')
fail_logger_length.setLevel(logging.INFO)
fail_logger_length.addHandler(logging.FileHandler(osp.join(save_path, 'tooFew_valid_frames_soNOdenoised.log')))
...
def denoising_by_length(ske_name, bodies_data):
    ...
    if len(bodies_data)==0:
        fail_logger_length.info(ske_name) # ntu数据集只有前50个动作类别是单人的
        return new_bodies_data, ''
...
def denoising_by_spread(ske_name, bodies_data):
    ...
    for (bodyID, body_data) in new_bodies_data.items():
        if len(bodies_data) == 1:
            break
        valid_frames = get_valid_frames_by_spread(body_data['joints'].reshape(-1, 17, 3))
        ...
        if ratio >= noise_spr_thres2:  # 0.69754
            ...
            joints = body_data['joints'].reshape(-1, 17, 3)[valid_frames]
...
def get_one_actor_points(body_data, num_frames):
    ...
    For joints, each frame contains 51 X-Y-Z coordinates.
    For colors, each frame contains 17 x 2 (X, Y) coordinates.
    """
    joints = np.zeros((num_frames, 51), dtype=np.float32)
    colors = np.ones((num_frames, 1, 17, 2), dtype=np.float32) * np.nan
...
def remove_missing_frames(ske_name, joints, colors):
    ...
    if num_bodies == 2:  # DEBUG
        missing_indices_1 = np.where(joints[:, :51].sum(axis=1) == 0)[0]
        missing_indices_2 = np.where(joints[:, 51:].sum(axis=1) == 0)[0]
...
def get_two_actors_points(bodies_data):
    ...
    # Arguments:
        ...
        the corresponding body_data which is also a dict with 4 keys:
          - joints: raw 3D joints positions. Shape: (num_frames x 17, 3)
          - colors: raw 2D color locations. Shape: (num_frames, 17, 2)
    ...
    ske_name = bodies_data['name']
    # label =  int(ske_name[ske_name.find('A')+1:ske_name.find('A')+4]) # ntu:int(ske_name[-2:])
    ...
    if len(bodies_data) == 1:  # Only left one actor after denoising
        ...
        #     fail_logger_2.info(ske_name) # ntu数据集只有前50个动作类别是单人的
...
def get_raw_denoised_data():
    ...
    For each frame of a skeleton sequence, an actor's 3D positions of 17 joints represented
    by an 2D array (shape: 17 x 3) is reshaped into a 51-dim vector by concatenating each
    3-dim (x, y, z) coordinates along the row dimension in joint order. Each frame contains
    two actor's joints positions constituting a 102-dim vector. If there is only one actor,
    then the last 51 values are filled with zeros. Otherwise, select the main actor and the
    second actor based on the motion amount. Each 102-dim vector as a row vector is put into
    ...
    frames_cnt = np.array(frames_cnt, dtype=int) # dtype=np.int
