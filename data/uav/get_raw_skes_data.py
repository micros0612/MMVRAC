...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    """
    ske_file = osp.join(skes_path, ske_name + '.txt') # +'.txt' OR '.skeleton'
    ...
    print('Reading data from %s' % ske_file[-51:]) # 文件名的长度
    ...
    for f in range(num_frames):
        ...
        valid_frames += 1
        joints = np.zeros((num_bodies, 17, 3), dtype=np.float32) # 三维坐标
        colors = np.zeros((num_bodies, 17, 2), dtype=np.float32) # 二维坐标


        for b in range(num_bodies):
            # 追踪的骨架编号, 
            # 若有一人, 则骨架编号为PersonID(str), 如 '0061'
            # 若有二人, 则骨架编号为PersonID(str), 如 '0061','0062'
            ...
            for j in range(num_joints):
                ...
                joints[b, j, :] = np.array(temp_str[:3], dtype=np.float32)  # 三维坐标
                colors[b, j, :] = np.array(temp_str[:2], dtype=np.float32) # 二维坐标, ntu是[5:7]第6个和第7个
                current_line += 1

            if bodyID not in bodies_data:  # Add a new body's data
                body_data = dict()
                body_data['joints'] = joints[b]  # ndarray: (17, 3)
                body_data['colors'] = colors[b, np.newaxis]  # ndarray: (1, 17, 2)
    ...
    if num_frames_drop > 0:
        frames_drop_skes[ske_name] = np.array(frames_drop, dtype=int)# dtype=np.int
...
def get_raw_skes_data():
    ...
    raw_skes_data = []
    frames_cnt = np.zeros(num_files, dtype=int)# dtype=np.int
...
if __name__ == '__main__':
    ...
    skes_path = '../uav_skeleton_raw/'  # ../nturgbd_raw/nturgb+d_skeletons/'
    stat_path = osp.join(save_path, 'statistics')
